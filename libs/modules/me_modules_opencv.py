...
def webcam_create_data(args):
    ...
    while count < 50:
        ...
        for (x, y, w, h) in faces:
            cv2.rectangle(
                im, (x, y), (x + w, y + h), (255, 0, 0), 2
            )
            face = gray[y : y + h, x : x + w]
            face_resize = cv2.resize(face, (width, height))
            cv2.imwrite(
                "% s/% s.png" % (path, count), face_resize
            )
        count += 1
...
def webcam_face_recognize(args):
    ...
    (images, labels) = [
        np.array(lis) for lis in [images, labels]
    ]
...
def NIRPlantVideoTracking(args):
    ...
    def create_colorbar(fig, image):
        ...
        cbar.update_ticks()
        cbar.set_label(
            "NDVI", fontsize=10, x=0.5, y=0.5, labelpad=-25
        )
    ...
    while 1:
        ...
        image = plt.imshow(
            ndvi, cmap=create_colormap(colors)
        )
        # plt.axis('off')
