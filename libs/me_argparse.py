...
def me_parse():  # noqa: WPS210 WPS213 WPS231 C901 CCR001 CFQ001
    ...
    lists_parser = subparsers.add_parser(
        "lists", help="lists commands"
    )

    ### DICTS

    # dicts ... dict based code
    dicts_parser = subparsers.add_parser(
        "dicts", help="dicts commands"
    )

    ### DECORATORS

    # decorators ... decorators based code
    decorators_parser = subparsers.add_parser(
        "decorators", help="decorator commands"
    )
    ...
    re_parser = subparsers.add_parser(
        "re", help="regex commands"
    )

    # import libs.me_scope
    scope_parser = subparsers.add_parser(
        "scope", help="Scope  commands"
    )

    # import libs.me_functions
    functions_parser = subparsers.add_parser(
        "functions", help="Function (builtin) commands"
    )

    # import libs.me_truthy
    truthy_parser = subparsers.add_parser(
        "truthy", help="Truthy commands"
    )

    # import libs.me_iterators
    iterators_parser = subparsers.add_parser(
        "iterators", help="Iterators commands"
    )

    # import libs.me_lambdas
    lambdas_parser = subparsers.add_parser(
        "lambdas", help="Lambdas commands"
    )

    # import libs.me_generators
    generators_parser = subparsers.add_parser(
        "generators", help="generators commands"
    )

    # import libs.me_tuples
    tuples_parser = subparsers.add_parser(
        "tuples", help="tuples commands"
    )

    # import libs.me_strings
    strings_parser = subparsers.add_parser(
        "strings", help="Strings commands"
    )

    # import libs.me_sets
    sets_parser = subparsers.add_parser(
        "sets", help="sets commands"
    )

    # import libs.me_exceptions
    exceptions_parser = subparsers.add_parser(
        "exceptions", help="Exceptions commands"
    )

    # import libs.me_filedir
    filedir_parser = subparsers.add_parser(
        "filedir", help="File and Directory commands"
    )

    # import libs.me_unix
    unix_parser = subparsers.add_parser(
        "unix", help="Unix commands"
    )

    # import libs.me_networking_IPC
    networking_parser = subparsers.add_parser(
        "networking", help="Networking and IPC commands"
    )

    # import libs.me_threading
    threading_parser = subparsers.add_parser(
        "threading", help="Threading commands"
    )

    # import libs.me_multiprocessing
    multiprocessing_parser = subparsers.add_parser(
        "multiprocessing", help="Multiprocessing commands"
    )

    # import libs.me_async
    async_parser = subparsers.add_parser(
        "async", help="async commands"
    )

    # import libs.me_coroutine
    coroutine_parser = subparsers.add_parser(
        "coroutine", help="coroutine commands"
    )

    # import libs.me_numerics
    numerics_parser = subparsers.add_parser(
        "numerics", help="numerics commands"
    )

    # import libs.me_comparisons
    comparisons_parser = subparsers.add_parser(
        "comparisons", help="comparison commands"
    )

    # import libs.me_compression
    compression_parser = subparsers.add_parser(
        "compression", help="Compression commands"
    )

    # import libs.me_internet
    internet_parser = subparsers.add_parser(
        "internet", help="Internet commands"
    )
    ...
    play_parser = subparsers.add_parser(
        "play", help="Play sub-command help"
    )

    # Create sub-sub parser for play sub-commands
    play_subparser = play_parser.add_subparsers(
        help="Play sub-sub-command help", dest="play_parser"
    )
    play_subparser_practice = play_subparser.add_parser(
        "practice", help="Play Practice sub-command help"
    )
    play_subparser_weather = play_subparser.add_parser(
        "weather", help="Play weather sub-command help"
    )
    ...
    modules_parser = subparsers.add_parser(
        "modules", help="Modules commands"
    )

    # Create sub-sub parser for module sub-commands
    modules_subparser = modules_parser.add_subparsers(
        help="Modules sub commands", dest="modules_parser"
    )

    # astral module sub-command
    modules_subparser_astral = modules_subparser.add_parser(
        "astral", help="Modules astral Sub Command"
    )

    # opencv module sub-command
    modules_subparser_opencv = modules_subparser.add_parser(
    ...
    modules_subparser_requests = (
        modules_subparser.add_parser(
            "requests", help="Modules Requests Sub Command"
        )
    )

    #  datetime module sub-command
    modules_subparser_datetime = (
        modules_subparser.add_parser(
            "datetime", help="Modules datetime Sub Command"
        )
    )

    # collections module sub-command
    modules_subparser_collections = (
        modules_subparser.add_parser(
            "collections",
    ...
    modules_subparser_heapq = modules_subparser.add_parser(
        "heapq", help="Modules heapq Sub Command"
    )

    # bisect  module sub-command
    modules_subparser_bisect = modules_subparser.add_parser(
        "bisect", help="Modules bisect Sub Command"
    )

    # types  module sub-command
    modules_subparser_types = modules_subparser.add_parser(
        "types", help="Modules types Sub Command"
    )

    # array  module sub-command
    modules_subparser_array = modules_subparser.add_parser(
        "array", help="Modules array Sub Command"
    )

    # copy  module sub-command
    modules_subparser_copy = modules_subparser.add_parser(
        "copy", help="Modules copy Sub Command"
    )

    # pprint  module sub-command
    modules_subparser_pprint = modules_subparser.add_parser(
        "pprint", help="Modules pprint Sub Command"
    )

    # math  module sub-command
    modules_subparser_math = modules_subparser.add_parser(
        "math", help="Modules math Sub Command"
    )

    # random  module sub-command
    modules_subparser_random = modules_subparser.add_parser(
        "random", help="Modules random Sub Command"
    )

    # statistics  module sub-command
    modules_subparser_statistics = (
        modules_subparser.add_parser(
            "statistics",
    ...
    modules_subparser_operator = (
        modules_subparser.add_parser(
            "operator", help="Modules operator Sub Command"
        )
    )

    # pathlib  module sub-command
    modules_subparser_pathlib = (
        modules_subparser.add_parser(
            "pathlib", help="Modules pathlib Sub Command"
        )
    )

    # os  module sub-command
    modules_subparser_os = modules_subparser.add_parser(
        "os", help="Modules os Sub Command"
    )

    #  glob module sub-command
    modules_subparser_glob = modules_subparser.add_parser(
        "glob", help="Modules glob Sub Command"
    )

    # fnmatch  module sub-command
    modules_subparser_fnmatch = (
        modules_subparser.add_parser(
            "fnmatch", help="Modules fnmatch Sub Command"
        )
    )

    # shutil  module sub-command
    modules_subparser_shutil = modules_subparser.add_parser(
        "shutil", help="Modules shutil Sub Command"
    )

    # sqlite3  module sub-command
    modules_subparser_sqlite3 = (
        modules_subparser.add_parser(
            "sqlite3", help="Modules sqlite3 Sub Command"
        )
    )

    # csv  module sub-command
    modules_subparser_csv = modules_subparser.add_parser(
        "csv", help="Modules csv Sub Command"
    )

    # json  module sub-command
    modules_subparser_json = modules_subparser.add_parser(
        "json", help="Modules json Sub Command"
    )

    # configparser  module sub-command
    modules_subparser_configparser = (
        modules_subparser.add_parser(
            "configparser",
    ...
    modules_subparser_io = modules_subparser.add_parser(
        "io", help="Modules io Sub Command"
    )

    # time  module sub-command
    modules_subparser_time = modules_subparser.add_parser(
        "time", help="Modules time Sub Command"
    )

    # errno  module sub-command
    modules_subparser_errno = modules_subparser.add_parser(
        "errno", help="Modules errno Sub Command"
    )

    # curses  module sub-command
    modules_subparser_curses = modules_subparser.add_parser(
        "curses", help="Modules curses Sub Command"
    )

    # ctypes  module sub-command
    modules_subparser_ctypes = modules_subparser.add_parser(
        "ctypes", help="Modules ctypes Sub Command"
    )

    # threading  module sub-command
    modules_subparser_threading = (
        modules_subparser.add_parser(
            "threading",
    ...
    modules_subparser_multiprocessing = (
        modules_subparser.add_parser(
            "multiprocessing",
    ...
    modules_subparser_sched = modules_subparser.add_parser(
        "sched", help="Modules sched Sub Command"
    )

    # queue  module sub-command
    modules_subparser_queue = modules_subparser.add_parser(
        "queue", help="Modules queue Sub Command"
    )

    # _thread  module sub-command
    modules_subparser__thread = (
        modules_subparser.add_parser(  # noqa: WPS116
            "_thread", help="Modules _thread Sub Command"
        )
    )

    # asyncio  module sub-command
    modules_subparser_asyncio = (
        modules_subparser.add_parser(
            "asyncio", help="Modules asyncio Sub Command"
        )
    )

    # socket  module sub-command
    modules_subparser_socket = modules_subparser.add_parser(
        "socket", help="Modules socket Sub Command"
    )

    # ssl  module sub-command
    modules_subparser_ssl = modules_subparser.add_parser(
        "ssl", help="Modules ssl Sub Command"
    )

    # mmap  module sub-command
    modules_subparser_mmap = modules_subparser.add_parser(
        "mmap", help="Modules mmap Sub Command"
    )

    #  signal module sub-command
    modules_subparser_signal = modules_subparser.add_parser(
        "signal", help="Modules signal Sub Command"
    )

    # urllib  module sub-command
    modules_subparser_urllib = modules_subparser.add_parser(
        "urllib", help="Modules urllib Sub Command"
    )

    # http  module sub-command
    modules_subparser_http = modules_subparser.add_parser(
        "http", help="Modules http Sub Command"
    )

    # socketserver  module sub-command
    modules_subparser_socketserver = (
        modules_subparser.add_parser(
            "socketserver",
    ...
    modules_subparser_turtle = modules_subparser.add_parser(
        "turtle", help="Modules turtle Sub Command"
    )

    # cmd  module sub-command
    modules_subparser_cmd = modules_subparser.add_parser(
        "cmd", help="Modules cmd Sub Command"
    )

    # shlex  module sub-command
    modules_subparser_shlex = modules_subparser.add_parser(
        "shlex", help="Modules shlex Sub Command"
    )

    # tkinter  module sub-command
    modules_subparser_tkinter = (
        modules_subparser.add_parser(
            "tkinter", help="Modules tkinter Sub Command"
        )
    )

    # typing  module sub-command
    modules_subparser_typing = modules_subparser.add_parser(
        "typing", help="Modules typing Sub Command"
    )

    # pydoc  module sub-command
    modules_subparser_pydoc = modules_subparser.add_parser(
        "pydoc", help="Modules pydoc Sub Command"
    )

    # pdb  module sub-command
    modules_subparser_pdb = modules_subparser.add_parser(
        "pdb", help="Modules pdb Sub Command"
    )

    # timeit  module sub-command
    modules_subparser_timeit = modules_subparser.add_parser(
        "timeit", help="Modules timeit Sub Command"
    )

    # sys  module sub-command
    modules_subparser_sys = modules_subparser.add_parser(
        "sys", help="Modules sys Sub Command"
    )

    # sysconfig  module sub-command
    modules_subparser_sysconfig = (
        modules_subparser.add_parser(
            "sysconfig",
    ...
    modules_subparser_inspect = (
        modules_subparser.add_parser(
            "inspect", help="Modules inspect Sub Command"
        )
    )

    # importlib  module sub-command
    modules_subparser_importlib = (
        modules_subparser.add_parser(
            "importlib",
        ...
        if args.sub_parser_name == "play":
            if not args.play_parser:
                ...
                play_parser.set_defaults(
                    module="scratchpad.practice", cmd="play"
                )
            ...
            lists_parser.set_defaults(
                func=libs.me_lists.me_lists, cmd="lists"
            )
        elif args.sub_parser_name == "dicts":
            import libs.me_dicts

            dicts_parser.set_defaults(
                func=libs.me_dicts.me_dicts, cmd="dicts"
            )
            ...
            re_parser.set_defaults(
                func=libs.me_re.me_re, cmd="re"
            )
        elif args.sub_parser_name == "scope":
            import libs.me_scope

            scope_parser.set_defaults(
                func=libs.me_scope.me_scope, cmd="scope"
            )
            ...
            truthy_parser.set_defaults(
                func=libs.me_truthy.me_truthy, cmd="truthy"
            )
            ...
            tuples_parser.set_defaults(
                func=libs.me_tuples.me_tuples, cmd="tuples"
            )
            ...
            sets_parser.set_defaults(
                func=libs.me_sets.me_sets, cmd="sets"
            )
            ...
            unix_parser.set_defaults(
                func=libs.me_unix.me_unix, cmd="unix"
            )
            ...
            async_parser.set_defaults(
                func=libs.me_async.me_async, cmd="async"
            )
