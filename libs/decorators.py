...
def log_output(fun, result: Any, kw: Dict, check_state: List) -> None:
    ...
    logger = logging.getLogger(fun.__name__)  # noqa: F841
...
def register(func):
    ...
    globals.PLUGINS[func.__name__] = func
    return func
...
def log_calls(logger, message="", sep=" "):
    """
    # https://www.bbkane.com/blog/logging-in-python/
    ...
    def wrap(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            argstr = [f"{arg}={value!r}" for arg, value in bound.arguments.items()]
            argstr = ", ".join(argstr)
